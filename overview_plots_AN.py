...
folders = []
folders = ['/Users/cesareborgia/cernbox/plots/plotter/2018/mmm/200310_13h_48m_CR_bj_no_svProb/datacards/',
           '/Users/cesareborgia/cernbox/plots/plotter/2018/mem_os/200310_13h_58m _CR_bj_no_svProb/datacards/',
...
for ch in chs:
    for m_bin in ['lo','hi']:
        for disp in disps:
            h_data.SetBinContent(i+1, yields['2018'][ch][disp][data][m_bin])
            h_prmp.SetBinContent(i+1, yields['2018'][ch][disp][prmp][m_bin])
            h_nonp.SetBinContent(i+1, yields['2018'][ch][disp][nonp][m_bin])

            h_data.SetBinError(i+1, yields_err['2018'][ch][disp][data][m_bin])
            h_prmp.SetBinError(i+1, yields_err['2018'][ch][disp][prmp][m_bin])
            h_nonp.SetBinError(i+1, yields_err['2018'][ch][disp][nonp][m_bin])
            
            i+=1

# stack = rt.THStack('bkg','bkg')
# stack.Add(nonp)
# stack.Add(prmp)
...
# high_flav = h_data.GetBinContent(h_data.GetMaximumBin()) * 1.145
high_flav = yaxis_max / 1.125
# high_mll = h_data.GetBinContent(h_data.GetMaximumBin()) * 1.2
...
state_0 = rt.TLatex(left_mll+2.05,  high_flav*.925, mmm)
state_0.SetTextFont(43)
state_0.SetTextSize(25)
# state_0.SetNDC()
state_0.Draw('same')

state_1 = rt.TLatex(left_mll+9.5,  high_flav*.925, mem_os)
state_1.SetTextFont(43)
state_1.SetTextSize(25)
# state_1.SetNDC()
state_1.Draw('same')

state_2 = rt.TLatex(left_mll+17.5, high_flav*.925, mem_ss)
